...
class BackgammonAITrainer {
    ...
    // ============================================
    addTrainingData(board, dice, turn, bestMoves) {
    ...
    calculateAccuracy(sample, weights) {
        const { board, dice, turn, bestMoves } = sample;

        const allMoves = getAvailableMoves(board, dice, turn);
        if (allMoves.length === 0) return 1;

        // ✅ اینجا selectBestMove خودش استراتژی و فاز را تشخیص می‌دهد
        // و از وزن‌های مربوطه استفاده می‌کند
        const bestMoveFromAI = aiService.selectBestMove(
            ...
            turn,
            '10',  // سطح 10 = بدون نویز
            null,  // strategyInfo = خودش تشخیص می‌دهد
            weights // وزن‌های سفارشی (که شامل همه استراتژی‌هاست)
        );
